...
const TABLES = [
  `CREATE TABLE IF NOT EXISTS folders (id TEXT PRIMARY KEY, slug TEXT UNIQUE NOT NULL, title TEXT, webhook_url TEXT, created_at TEXT DEFAULT (datetime('now')))`,
...
export function request(path: string, init?: RequestInit) {
  ...
  return app.fetch(req, { DB: db, ASSETS: { fetch: () => new Response("<!DOCTYPE html><html><head></head><body></body></html>") } as any })
}
